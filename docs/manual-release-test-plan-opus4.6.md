...
| 7.2 | Relay event in stream       | `rm -f /tmp/evb-relay-watch.ndjson; evb-relay input watch --robot >/tmp/evb-relay-watch.ndjson 2>/dev/null & watch_pid=$!; sleep 1; evb-relay relay toggle onboard:1 --format json >/dev/null; sleep 2; kill "$watch_pid"; wait "$watch_pid" 2>/dev/null || true; jq -s -e 'any(.[]; .event=="relay_changed" and .data.group=="onboard" and .data.id==1 and (.data.state \| type)=="boolean")' /tmp/evb-relay-watch.ndjson >/dev/null` | Command exits 0 only if captured NDJSON includes a `relay_changed` event for onboard relay 1 with a boolean `state` | | |
...
| 7.5 | Multiple SSE clients        | `rm -f /tmp/evb-events-a.sse /tmp/evb-events-b.sse; curl -sfS -N -H "Authorization: Bearer $EVB_RELAY_API_TOKEN" http://$EVB_RELAY_HOST/api/v1/events >/tmp/evb-events-a.sse 2>/dev/null & pid_a=$!; curl -sfS -N -H "Authorization: Bearer $EVB_RELAY_API_TOKEN" http://$EVB_RELAY_HOST/api/v1/events >/tmp/evb-events-b.sse 2>/dev/null & pid_b=$!; sleep 1; evb-relay relay toggle onboard:1 --format json >/dev/null; sleep 2; kill "$pid_a" "$pid_b"; wait "$pid_a" "$pid_b" 2>/dev/null || true; grep -q 'event: relay_changed' /tmp/evb-events-a.sse; grep -q 'data: {\"group\":\"onboard\",\"id\":1' /tmp/evb-events-a.sse; grep -q 'event: relay_changed' /tmp/evb-events-b.sse; grep -q 'data: {\"group\":\"onboard\",\"id\":1' /tmp/evb-events-b.sse` | Command exits 0 only if both SSE capture files contain the relay-change event for onboard relay 1 | | |
...
| #   | Test                        | Command | Expected | Result | Notes |
|-----|-----------------------------|---------|----------|--------|-------|
| 17.1 | Start event capture        | `rm -f /tmp/evb-button-watch.ndjson; timeout 15 evb-relay input watch --robot 2>/dev/null > /tmp/evb-button-watch.ndjson & watch_pid=$!; sleep 1; test -f /tmp/evb-button-watch.ndjson; sed -n '1p' /tmp/evb-button-watch.ndjson \| jq -e '.stream=="events" and (.device_context \| type)=="object"' >/dev/null; kill -0 "$watch_pid"` | Capture file exists, the first line is the NDJSON stream header, and the background watch is still alive when 17.2 begins | | |
