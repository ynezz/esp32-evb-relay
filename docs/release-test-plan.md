...
| #   | Test                        | Command | Expected | Result | Notes |
|-----|-----------------------------|---------|----------|--------|-------|
| 7.1 | Input watch starts          | `timeout 10 evb-relay input watch --robot 2>/dev/null \| head -3` | | | Stream header with v=1, stream=events, device_context |
...
| 7.4 | SSE via REST                | `timeout 5 curl -s -N .../events \| head -5` | | | SSE format with `:connected` and event/data lines |
...
| 10.7  | X-ModIO-Present header      | `curl ... \| grep -i 'X-ModIO-Present'` | | | X-ModIO-Present: false |
| 10.8  | X-ModIO-Sync header         | `curl ... \| grep -i 'X-ModIO-Sync'` | | | X-ModIO-Sync: absent |
